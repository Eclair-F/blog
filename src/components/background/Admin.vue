<template>
	<div>
		...
		<el-card class="card">
			...
			<!-- 表格区域 -->
			<el-table border style="width: 100%" class="tables" :data="Adminslist">
				...
				<el-table-column label="操作" width="180px">
					<template slot-scope="scope">
						...
						<el-tooltip class="item" effect="dark" content="编辑" placement="top-start" :enterable="false">
							<el-button type="primary" icon="el-icon-edit" @click="alterAdmin(scope.row.id)" size="small"></el-button>
						</el-tooltip>
						<el-tooltip class="item" effect="dark" content="删除" placement="top-start" :enterable="false">
							<el-button type="danger" icon="el-icon-delete" @click="delectAdmin(scope.row.id)" size="small"></el-button>
						</el-tooltip>
						<el-tooltip class="item" effect="dark" content="提升权限" placement="top-start" :enterable="false">
		...
		<el-dialog title="修改用户信息" :visible.sync="updateDialogVisible" @close="updateClosed" width="40%">
			<el-form label-width="80px" :model="alerlist" :rules="addformRule" ref="uodateref">
				<el-form-item label="用户名" prop=""><el-input v-model="alerlist.username" disabled></el-input></el-form-item>
		...
		<!-- 降低权限对话框 -->
		<el-dialog title="降低权限" :visible.sync="allotJurisdictionCase" @close="roleClosed" width="40%">
...
export default {
	data() {
		...
		//   验证用户名
		const checkAdminname = async (rule, value, cb) => {
	...
	methods: {
		...
		pageUp() {
			this.state = 'up';
			this.query.id = this.Adminslist[0].id;
			this.currentPage -= 1;
			this.down = true;
			this.getAdminList();
		},
		...
		pageDown() {
			this.state = 'down';
			this.query.id = this.Adminslist[this.Adminslist.length - 1].id;
			this.currentPage += 1;
			this.up = true;
			this.getAdminList();
		},
